...
/** Master cấp TDMA: 10 slot × 5 ms = 50 ms/khung (tối đa 10 slave IMU đồng thời). */
#define MASTER_IMU_TX_SLOT_COUNT 10U
#define MASTER_SYNC_SLOT_COUNT 11U
#define MASTER_SLOT_UNASSIGNED 0xFFU
/** packet_t.slot_flags: Master đã gán imu_tx_slot / sync_slot hợp lệ. */
...
typedef struct {
  ...
  /** Master → Slave trong ACK/TIME (firmware slave mới). */
  uint8_t imu_tx_slot;
  uint8_t sync_slot;
...
/** Kích thước packet_t trước khi thêm trường slot (slave/Master cũ). */
